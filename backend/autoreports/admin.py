...
class ReportAdmin(ReportApi):
    ...
    is_admin = True
    ReportChangeList = AutoReportChangeList
    ...
    def _get_change_list(self, request, model, cl_options, report):
        ...
        cl = self.ReportChangeList(request, model, prefix_url, report, list_display, list_display_links, list_filter,
            date_hierarchy, search_fields, list_select_related, list_per_page, 200, list_editable, model_admin)
        cl.formset = None
        return cl
    ...
    def report_list(self, request, extra_context=None):
        ...
        cl = context['cl']
        cl.queryset = cl.queryset.filter(content_type=content_type)
    ...
    def report_quick(self, request):
        ...
        try:
            try:
                ...
                cl = ChangeList(request, self.model, self.list_display, self.list_display_links, self.list_filter,
                    self.date_hierarchy, self.search_fields, self.list_select_related, self.list_per_page, self.list_editable, 200, self)
