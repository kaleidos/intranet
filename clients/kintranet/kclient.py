...
class UtilsAuthenticationMixin():
    ...
    def save_token(self, user):
        self.session.headers.update({
                        settings.HTTP_TOKEN: user['token']})

    def logout(self):
         r = self.session.post(
                settings.BASE_URL + 'auth/logout/')
...
class UtilsPartsMixin():
    ...
    def get_projects(self):
        return self.session.get(
                ...
                params={'active': True}).json()


    def get_part(self, part_id):
        return self.session.get(
                settings.BASE_URL + 'parts/' + part_id).json()
...
class UtilsHolidaysMixin():
    ...
    def view_my_holidays(self):
        ...
        for h in holidays:
            ...
            line = [str(year), str(total_days),
                    str(consumed_days), str(requested_days),
                    str(pending_days)]
    ...
    def get_holidays(self):
        return self.session.get(
                settings.BASE_URL + 'holidays-years/').json()
...
class KClient(UtilsAuthenticationMixin,
              UtilsPartsMixin,
              UtilsHolidaysMixin):
    def __init__(self):
        self.session = KSession()
        self.session.headers.update({
